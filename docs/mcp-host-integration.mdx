...
<Warning>
  **If you front the server with a static host, `/mcp` must be rewritten to it.**
  Only the paths a static host is told to forward reach the server; everything
  else falls through to the console's own page. Measured 2026-09-06 on the
  maintainers' deployment, before the rule was applied: `/mcp` answered
  `200 text/html` — the console — so an MCP host pointed at it received HTML
  where it expects JSON-RPC, and the failure looked like a broken server rather
  than a routing rule. The paths that must cross are `/api/**`, `/mcp`,
  `/mcp/**`, `/share`, `/share/**` and `/handoff`.
</Warning>
