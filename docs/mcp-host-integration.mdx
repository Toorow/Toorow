...
|  |  - Daily Insight Composer      |   |  - Dual-Channel Response (Text +   |  |
...
- **Transport Endpoint**: `http://localhost:8000/mcp` (local dev), or `/mcp` on
  the host that serves the toorow server itself in production.

<Warning>
  **Point the host at the server, not at the console's domain.** If you put a
  static host in front of the console, only the paths it is told to rewrite reach
  the server. Measured 2026-09-06 on the maintainers' deployment: `/api/**`,
  `/share`, `/share/**` and `/handoff` are rewritten, `/mcp` is not — so
  `https://app.toorow.com/mcp` answers `200 text/html`, the console's own page. An
  MCP host pointed there receives HTML where it expects JSON-RPC, and the failure
  looks like a broken server rather than a routing rule. Either add `/mcp` to the
  rewrites, or give the host the server's own address.
</Warning>
...
```json
{
  "mcpServers": {
    "toorow-prod": {
      "url": "https://<your-toorow-server>/mcp",
      "headers": {
