...
A host does not see all 147 tools. It sees the profiles its session is entitled
to, and nothing else.

| Profile | Tools | What it may do | Reached when |
| --- | --- | --- | --- |
| `insights` | 60 | Reads only. Never mutates. | Always — this is the floor, with no token and no proof of workspace |
| `operations` | 47 | Runs, drafts, schedules, compositions | The session proves an endpoint and a workspace |
| `governance` | 40 | Publishes definitions, capabilities and master data | The session proves an endpoint, a workspace, and verified interactive presence |
