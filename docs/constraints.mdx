...
<Warning>
Compliance with these constraints is mandatory for all deployments and connectors. Any public exposure of an instance running in `disabled` or `static` auth mode is strictly prohibited.
</Warning>
...
4. **Multi-Tenant Isolation**: Every resource request resolves through an organization-rooted access graph. Tenant isolation is enforced at the database and warehouse queries level and validated by CI isolation test suites.
