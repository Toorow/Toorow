...
description: Interactive workflows demonstrating how AI agents trigger tools that render embedded MCP App UI widgets directly inside host client interfaces.
---
...
- **Agent Action**: Executes `get_daily_report(project_id)`, then `preview_daily_insight(project_id, payload)` to compose the narrative that goes with the numbers.
...
- **Agent Action**: Executes `get_daily_report(project_id, connectors=["meta-ads", "tiktok-ads", "google-analytics"])`.
