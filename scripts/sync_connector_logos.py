...
SHARED_MARKS = {
    ...
    frozenset({"instagram-insights", "meta-ads"}),
}
...
def _canonical_plan() -> tuple[dict[str, str], dict[str, str], dict[str, str]]:
    """Return (files, resolver_map, names_map).

    ``files``        : filename -> absolute canonical source path (deduped).
    ``resolver_map`` : slug -> "/connectors/<file>" keyed by identity id AND stem.
    ``names_map``    : slug -> publicName, same keys as ``resolver_map``. A screen
                       that shows a connector needs the label and the mark
                       together; a hand-written label table drifts from the logos.

    NB: this signature returns THREE maps. It returned two until ``names_map``
    was added alongside ``_serialize_map(resolver, names)``, and the annotation
    was left saying two -- which is why four contract tests unpacked two and
    failed with "too many values to unpack" instead of being caught by a type
    check. Keep the annotation and the ``return`` in step.
    ...
    for connector_id in sorted(identities):
        ...
        # Aliases: provider strings the product stores that are not the
        # identity id (`ga4` for Google Analytics, `facebook` for Meta Ads).
        # They belong here, once, and not in a per-screen lookup table where a
        # screen has to guess which vendor a string means.
        aliases = entry.get("aliases") or []
        if not isinstance(aliases, list) or not all(isinstance(a, str) for a in aliases):
            raise LogoSyncError(f"{connector_id}: $.aliases must be a list of strings")

        public_name = entry.get("publicName")
        if isinstance(public_name, str) and public_name:
            for key in (connector_id, *aliases):
                names.setdefault(_slug(key), public_name)

        if asset.get("kind") == "fallback":
            for key in (connector_id, *aliases):
                resolver[_slug(key)] = "/connectors/generic.svg"
            continue
        ...
        for alias in aliases:
            resolver[_slug(alias)] = local_path
    ...
    return files, resolver, names


def _serialize_map(resolver: dict[str, str], names: dict[str, str]) -> bytes:
    payload = {
        "contractVersion": "1",
        "_note": (
            "GENERATED by scripts/sync_connector_logos.py from"
            " web/src/data/connector-identities.json. Do not edit by hand."
        ),
        "logos": {slug: resolver[slug] for slug in sorted(resolver)},
        # The display name for the same key. A screen that shows a connector
        # needs both, and hand-written label tables drift from the logos.
        "names": {slug: names[slug] for slug in sorted(names)},
    }
    return (json.dumps(payload, ensure_ascii=False, indent=2) + "\n").encode("utf-8")


def _drift(files: dict[str, str], resolver: dict[str, str], names: dict[str, str]) -> list[str]:
    ...
    expected_map = _serialize_map(resolver, names)
...
def _apply(files: dict[str, str], resolver: dict[str, str], names: dict[str, str]) -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        files, resolver, names = _canonical_plan()
    except LogoSyncError as exc:
        print(str(exc), file=sys.stderr)
        return 2

    if args.check:
        problems = _drift(files, resolver, names)
        if problems:
    ...
    actions = _apply(files, resolver, names)
    if actions:
        for line in actions:
            print(line)
