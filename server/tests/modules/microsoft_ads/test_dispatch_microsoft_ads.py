"""Story 26.4 -- AI-58 dispatch: all 6 microsoft-ads profiles resolve (AC).
...
unknown/unavailable profiles. Pattern: google-ads test_dispatch (26.2).
"""
...
_PROFILE_CALLABLES = {
    ...
    "search_query_daily": "pull_search_query_daily",
...
def test_each_profile_dispatches_to_its_declared_callable(profile_id, callable_name):
    ...
    with patch("core.main._loaded_modules", [loaded]):
        fn = get_module_pull_fn("microsoft-ads", profile_id)
...
def test_default_dispatch_is_bare_pull():
    mod, loaded = _loaded()
    with patch("core.main._loaded_modules", [loaded]):
        fn = get_module_pull_fn("microsoft-ads", None)
...
def test_unknown_profile_fails_closed():
    _mod, loaded = _loaded()
    with patch("core.main._loaded_modules", [loaded]):
        assert get_module_pull_fn("microsoft-ads", "nope_daily") is None
