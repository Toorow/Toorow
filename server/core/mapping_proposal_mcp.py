...
def inspect_mapping(datastream_id: str, mapping_version_id: str | None = None):
    ...
    if version is None:
        data = {
            ...
            "note": "No current mapping version.",
        }
        return _result("No current mapping version for this Datastream.", data)
    ...
    summary = (
        f"Mapping v{version.get('version_number')} of the datastream: "
        f"{len(fields)} field(s), {blocking} blocking, "
        f"{len(ambiguities)} ambiguity(ies)."
    )
...
def propose_mapping_correction(
    ...
    if plan_version_id is None:
        raise _tool_error("missing_param", "plan_version_id is required.")
    if idempotency_key is None:
        raise _tool_error("missing_param", "idempotency_key is required.")
    if not isinstance(mapping_payload, dict):
        raise _tool_error("invalid_param", "mapping_payload must be an object.")
    ...
    if not mode_permits(mode, effect="persist"):
        raise _tool_error(
            "mode_denied",
            "The current agent mode only allows explaining, not proposing.",
        )
    ...
    try:
        ...
        raise _tool_error(
            "candidate_rejected",
            "Candidate rejected: a check failed; the published mapping is unchanged.",
        ) from _StructuredRejection(data)
    except MappingProposalModeDenied as exc:
        raise _tool_error(
            "mode_denied",
            "The current agent mode does not allow proposing.",
        ) from exc
    except MappingProposalUnavailable as exc:
        raise _tool_error("server_error", "The proposal could not be recorded.") from exc
    except Exception as exc:  # noqa: BLE001
    ...
    summary = (
        f"Proposal {result.proposal_id} recorded (v{result.version_number}, "
        f"state 'ready', NOT LIVE); routed to the governed review (Story 36.18). "
        f"The published mapping is unchanged."
    )
    return _result(summary, data)
...
def test_mapping_candidate(datastream_id: str, mapping_payload: dict, plan_version_id: str):
    ...
    if not mode_permits(mode, effect="test"):
        raise _tool_error(
            "mode_denied",
            "Candidate testing is only allowed in Autonomous-within-policy mode.",
        )
    ...
    verdict = "all checks pass" if all_pass else "check(s) failed"
    summary = (
        f"Test candidat : {verdict} "
        f"(no version inserted, no pointer advanced)."
    )
    return _result(summary, data)
...
def register(mcp) -> None:
    ...
    register_profiled(
        ...
        profile="operations",
        # A candidate test writes evidence and authorizes nothing: it is a prepare.
        effect="prepare",
        data_class="operational",
